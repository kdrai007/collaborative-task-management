...
export const columnApi = {
  ...
  update: (workspaceId: string, columnId: string, data: { title: string }) =>
    apiFetch<{ column: Column }>(`/workspaces/${workspaceId}/columns/${columnId}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
...
export const taskApi = {
  ...
  create: (
    workspaceId: string,
    data: { columnId: string; title: string; description?: string; priority?: Task['priority']; assigneeId?: string | null; dueDate?: string | null },
  ) =>
  ...
  update: (
    workspaceId: string,
    taskId: string,
    data: Partial<Pick<Task, 'title' | 'description' | 'priority' | 'status' | 'assigneeId' | 'dueDate' | 'columnId'>>,
  ) =>
    apiFetch<{ task: Task }>(`/workspaces/${workspaceId}/tasks/${taskId}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
